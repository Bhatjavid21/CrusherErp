...
public class H_tbl_Sales : IHttpHandler, IRequiresSessionState
{
    ...
    void ListAllSales(string SearchString,int Page_No)
    {
        ...
        string  sql = "with NewTable as (select a.*,ROW_NUMBER() over (order by Id desc) as RowNum from tbl_Sales a " +
               "  where 1=1 "+SearchFilter+") select * from NewTable where RowNum between "+from+" And "+to;

        DataTable dt=DB.GetDataTable(sql);
        TotalRecords=DB.Get_ScalerInt("select count(a.Id) from tbl_Sales a where 1=1  "+SearchFilter);
    ...
    void Save_Sales(string InsertArray)
    {
        ...
        string sql = "Insert into tbl_Sales values('" + Data[0] + "','" + Data[1] + "','" + Data[3] + "','" + DateTime.Now.ToString("yyyy-MM-dd")+ "','" + Data[2] + "',null)";
