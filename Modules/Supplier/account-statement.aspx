            ...
            <table class="" style="width: 100%">
                <tbody >
                   ...
                   <br />
                    ...
                    <tr>
                        <td colspan="9">[[[<asp:LinkButton ID="btnback" PostBackUrl="~/Modules/Supplier/Supplier.aspx" CssClass="btn btn-success" runat="server" Text="Back" />]]]
                        </td>
    ...
    <script>
        $(document).ready(function () {
            if (window.location.search.includes('?')) {
                ...
                SupplierId = urlParams.get('Sup_id');
                var btDate = urlParams.get('Date');
                var DivId = urlParams.get('DivId');

                GetMetaData(SupplierId, btDate, DivId)
                GetAccountData(SupplierId, btDate);
                GetAgingAnalysisData(SupplierId, btDate);
        ...
        function Excel_Data() {
            ...
            var urlParams = new URLSearchParams(window.location.search);
            var SupplierId = urlParams.get('Sup_id');
            var btDate = urlParams.get('Date');
            var DivId = urlParams.get('DivId');
            //alert(CustomeId)
            $.ajax({
                ...
                type: "POST",
                data: {
                    'fun': 'ExportExcelSOA', 'SupplierId': SupplierId, 'date': btDate
                },
        ...
        function GetMetaData(SupplierId, btDate, division_Id) {
          
            $.ajax({
                url: 'H_tbl_Supplier.ashx',
                type: "POST",
                data: {
                    'fun': 'GetMatadataforPdf', 'SupplierId': SupplierId, 'division_Id': division_Id
                },
                success: function (data) {
                    if (Chk_Res(data.errorMessage) == false) {
                        var arr = data.split('|');
                        ...
                        $.each(SupplierData, function (i, obj) {
                            ...
                            SetInnerVal("CusAddress", " <b> " + obj.Supplier_Name + " <br /> " + obj.Address + "    </b>");
                            $('#FileNameField').val("Stament Of Account of " + obj.Supplier_Name) 
        ...
        function GetAccountData(SupplierId, btDate) {
            $('#tblAccountStatementbody').empty();
            $.ajax({
                url: 'H_tbl_Supplier.ashx',
                type: "POST",
                data: {
                    'fun': 'GetAccountData', 'SupplierId': SupplierId, 'date': btDate
                },
                success: function (data) {
                    if (Chk_Res(data.errorMessage) == false) {
                        if (data.toString() != "") {
        ...
        function GetAgingAnalysisData(SupplierId, btDate) {
            $('#Ageing').empty();
            $.ajax({
                url: 'H_tbl_Supplier.ashx',
                type: "POST",
                data: {
                    'fun': 'GetAgingAnalysisData', 'SupplierId': SupplierId, 'date': btDate
                },
                success: function (data) {
                    if (Chk_Res(data.errorMessage) == false) {
                        if (data.toString() != "") {
